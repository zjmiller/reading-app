import batAudioUrl from "../assets/sounds/bat.m4a";
import beeAudioUrl from "../assets/sounds/bee.m4a";
import birdAudioUrl from "../assets/sounds/bird.m4a";
import catAudioUrl from "../assets/sounds/cat.m4a";

import cowAudioUrl from "../assets/sounds/cow.m4a";

import crabAudioUrl from "../assets/sounds/crab.m4a";

import dinosaurAudioUrl from "../assets/sounds/dinosaur.m4a";

import dogAudioUrl from "../assets/sounds/dog.m4a";
import fishAudioUrl from "../assets/sounds/fish.m4a";
import foxAudioUrl from "../assets/sounds/fox.m4a";
import frogAudioUrl from "../assets/sounds/frog.m4a";

import iglooAudioUrl from "../assets/sounds/igloo.m4a";

import jumpAudioUrl from "../assets/sounds/jump.m4a";
import pigAudioUrl from "../assets/sounds/pig.m4a";
import ratAudioUrl from "../assets/sounds/rat.m4a";
import runAudioUrl from "../assets/sounds/run.m4a";
import sharkAudioUrl from "../assets/sounds/shark.m4a";

import snakeAudioUrl from "../assets/sounds/snake.m4a";

import sunAudioUrl from "../assets/sounds/sun.m4a";

import wormAudioUrl from "../assets/sounds/worm.m4a";

import nestAudioUrl from "../assets/sounds/nest.m4a";
// import shellAudioUrl from "../assets/sounds/shell.m4a";
import oneAudioUrl from "../assets/sounds/one.m4a";
import shopAudioUrl from "../assets/sounds/shop.m4a";
import saidAudioUrl from "../assets/sounds/said.m4a";
import askAudioUrl from "../assets/sounds/ask.m4a";
import bestAudioUrl from "../assets/sounds/best.m4a";
import deskAudioUrl from "../assets/sounds/desk.m4a";
import dustAudioUrl from "../assets/sounds/dust.m4a";
import fastAudioUrl from "../assets/sounds/fast.m4a";
import justAudioUrl from "../assets/sounds/just.m4a";
import lastAudioUrl from "../assets/sounds/last.m4a";
import listAudioUrl from "../assets/sounds/list.m4a";
import maskAudioUrl from "../assets/sounds/mask.m4a";
import mustAudioUrl from "../assets/sounds/must.m4a";
import onceAudioUrl from "../assets/sounds/once.m4a";
import pastAudioUrl from "../assets/sounds/past.m4a";
import restAudioUrl from "../assets/sounds/rest.m4a";
import testAudioUrl from "../assets/sounds/test.m4a";
import shotAudioUrl from "../assets/sounds/shot.m4a";

export default [
  {
    id: "word-1",
    word: "cat",
    audio: catAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "6uMqzcbWRhoT6",
        iframeWidth: "480",
        iframeHeight: "360",
        aHref: "https://giphy.com/gifs/cat-animal-kitten-6uMqzcbWRhoT6",
      },

      {
        iframeSrc: "VxbvpfaTTo3le",
        iframeWidth: "480",
        iframeHeight: "262",
        aHref: "https://giphy.com/gifs/cat-kitten-kitty-VxbvpfaTTo3le",
      },

      {
        iframeSrc: "MDJ9IbxxvDUQM",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/cat-kisses-hugs-MDJ9IbxxvDUQM",
      },
    ],
  },

  {
    id: "word-2",
    word: "dog",
    audio: dogAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "l41m0CPz6UCnaUmxG",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref:
          "https://giphy.com/gifs/tired-sleepy-goodnight-l41m0CPz6UCnaUmxG",
      },

      {
        iframeSrc: "oebOcslmnSXMQ",
        iframeWidth: "480",
        iframeHeight: "237",
        aHref: "https://giphy.com/gifs/july-great-oebOcslmnSXMQ",
      },

      {
        iframeSrc: "EAbMgjdtduMdq",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/dog-day-yr-EAbMgjdtduMdq",
      },
    ],
  },

  {
    id: "word-3",
    word: "fox",
    audio: foxAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "LvHBV2O3aFqBG",
        iframeWidth: "480",
        iframeHeight: "258",
        aHref:
          "https://giphy.com/gifs/headlikeanorange-fox-red-springwatch-LvHBV2O3aFqBG",
      },

      {
        iframeSrc: "qkdTy6tTmF7Xy",
        iframeWidth: "480",
        iframeHeight: "342",
        aHref: "https://giphy.com/gifs/fox-qkdTy6tTmF7Xy",
      },

      {
        iframeSrc: "xsE65jaPsUKUo",
        iframeWidth: "480",
        iframeHeight: "349",
        aHref: "https://giphy.com/gifs/fox-animal-kawaii-xsE65jaPsUKUo",
      },
    ],
  },

  {
    id: "word-4",
    word: "pig",
    audio: pigAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "WsVJuYCz6qgJG",
        iframeWidth: "480",
        iframeHeight: "276",
        aHref: "https://giphy.com/gifs/cute-video-animal-WsVJuYCz6qgJG",
      },

      {
        iframeSrc: "5odBbOYQN3UaY",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/pig-little-piggeeehh-5odBbOYQN3UaY",
      },

      {
        iframeSrc: "HV6uVZWJDJ42c",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/swimming-pig-HV6uVZWJDJ42c",
      },

      {
        iframeSrc: "E5TVttEnaz4ME",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/E5TVttEnaz4ME",
      },
    ],
  },

  {
    id: "word-5",
    word: "bat",
    audio: batAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "Wm3mmcfsEnqaA",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/bat-gum-Wm3mmcfsEnqaA",
      },

      {
        iframeSrc: "X1zfywreqWqIw",
        iframeWidth: "480",
        iframeHeight: "280",
        aHref: "https://giphy.com/gifs/X1zfywreqWqIw",
      },
    ],
  },

  {
    id: "word-6",
    word: "rat",
    audio: ratAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "gOH54eiriYIwM",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/costume-gOH54eiriYIwM",
      },

      {
        iframeSrc: "q15EjVC1dBbOM",
        iframeWidth: "480",
        iframeHeight: "363",
        aHref: "https://giphy.com/gifs/q15EjVC1dBbOM",
      },
    ],
  },

  {
    id: "word-7",
    word: "bee",
    audio: beeAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "SSUyICBgb3tYI",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/bees-SSUyICBgb3tYI",
      },

      {
        iframeSrc: "5HXevEAuRyily",
        iframeWidth: "480",
        iframeHeight: "374",
        aHref: "https://giphy.com/gifs/5HXevEAuRyily",
      },
    ],
  },

  {
    id: "word-8",
    word: "frog",
    audio: frogAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "ytAW7lMP77p5e",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/bbc-frog-amphibian-ytAW7lMP77p5e",
      },

      {
        iframeSrc: "4N4JBFo45PVPa",
        iframeWidth: "480",
        iframeHeight: "258",
        aHref: "https://giphy.com/gifs/tongue-frog-insect-4N4JBFo45PVPa",
      },
    ],
  },

  {
    id: "word-9",
    word: "sun",
    audio: sunAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "3ogwG0cKu7x87xU6VG",
        iframeWidth: "480",
        iframeHeight: "386",
        aHref:
          "https://giphy.com/gifs/feistmusic-century-feist-3ogwG0cKu7x87xU6VG",
      },

      {
        iframeSrc: "l0ExbqkAKqIrlWdI4",
        iframeWidth: "480",
        iframeHeight: "365",
        aHref: "https://giphy.com/gifs/l0ExbqkAKqIrlWdI4",
      },
    ],
  },

  {
    id: "word-10",
    word: "run",
    audio: runAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "l2Sqc3POpzkj5r8SQ",
        iframeWidth: "480",
        iframeHeight: "201",
        aHref: "https://giphy.com/gifs/run-forrest-gump-l2Sqc3POpzkj5r8SQ",
      },

      {
        iframeSrc: "26FKYPQg5CUfV2ipa",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref:
          "https://giphy.com/gifs/animation-run-super-deluxe-26FKYPQg5CUfV2ipa",
      },
    ],
  },

  {
    id: "word-11",
    word: "jump",
    audio: jumpAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "l2JJIfI6nUkm5AzoQ",
        iframeWidth: "480",
        iframeHeight: "240",
        aHref: "https://giphy.com/gifs/l2JJIfI6nUkm5AzoQ",
      },

      {
        iframeSrc: "3o6Ztroynp3WpqLD1u",
        iframeWidth: "480",
        iframeHeight: "240",
        aHref: "https://giphy.com/gifs/bbc-wildlife-3o6Ztroynp3WpqLD1u",
      },
    ],
  },

  {
    id: "word-12",
    word: "bird",
    audio: birdAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "l1J9uG4jNOSsYtbkA",
        iframeWidth: "480",
        iframeHeight: "285",
        aHref:
          "https://giphy.com/gifs/friday-shake-it-off-shoebill-stork-l1J9uG4jNOSsYtbkA",
      },

      {
        iframeSrc: "efqBQ5OOnQehq",
        iframeWidth: "480",
        iframeHeight: "258",
        aHref: "https://giphy.com/gifs/bird-standing-efqBQ5OOnQehq",
      },
    ],
  },

  {
    id: "word-13",
    word: "fish",
    audio: fishAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "7eVp9MHlNI90c",
        iframeWidth: "480",
        iframeHeight: "380",
        aHref: "https://giphy.com/gifs/fish-playful-splashing-7eVp9MHlNI90c",
      },

      {
        iframeSrc: "sxXJgb4LCmJ44",
        iframeWidth: "480",
        iframeHeight: "117",
        aHref: "https://giphy.com/gifs/clown-fish-sxXJgb4LCmJ44",
      },
    ],
  },

  {
    id: "word-14",
    word: "shark",
    audio: sharkAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "nDhXcwufoptC0",
        iframeWidth: "480",
        iframeHeight: "284",
        aHref: "https://giphy.com/gifs/fish-playful-splashing-7eVp9MHlNI90c",
      },

      {
        iframeSrc: "SZzKe0HhRlthC",
        iframeWidth: "480",
        iframeHeight: "230",
        aHref: "https://giphy.com/gifs/shark-shortfin-mako-SZzKe0HhRlthC",
      },
    ],
  },

  {
    id: "word-15",
    word: "crab",
    audio: crabAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "odwOXY8XpcYWQ",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/crab-crustatean-gif-odwOXY8XpcYWQ",
      },

      {
        iframeSrc: "l2Sq6SNEYMDQ8NtdK",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/originals-l2Sq6SNEYMDQ8NtdK",
      },
    ],
  },

  {
    id: "word-16",
    word: "igloo",
    chestMin: "RARE",
    audio: iglooAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "UKyqX1eV8Tgl2",
        iframeWidth: "480",
        iframeHeight: "360",
        aHref: "https://giphy.com/gifs/satisfying-igloo-UKyqX1eV8Tgl2",
      },

      {
        iframeSrc: "l2Sq6SNEYMDQ8NtdK",
        iframeWidth: "480",
        iframeHeight: "276",
        aHref: "https://giphy.com/gifs/eskimo-ni2wk9VTYMw9O",
      },
    ],
  },

  {
    id: "word-17",
    word: "dinosaur",
    chestMin: "EPIC",
    audio: dinosaurAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "fRzQRQ2Fux1Is",
        iframeWidth: "480",
        iframeHeight: "248",
        aHref:
          "https://giphy.com/gifs/animated-dinosaur-brontosaurus-fRzQRQ2Fux1Is",
      },

      {
        iframeSrc: "iVqXV4vE4DDYk",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/dinosaur-iVqXV4vE4DDYk",
      },
    ],
  },

  {
    id: "word-18",
    word: "snake",
    chestMin: "RARE",
    audio: snakeAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "jCOyMYDIe7Yd2",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/snake-jCOyMYDIe7Yd2",
      },

      {
        iframeSrc: "PL3xU8RKdMfF6",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/snake-PL3xU8RKdMfF6",
      },
    ],
  },

  {
    id: "word-19",
    word: "cow",
    chestMin: "RARE",
    audio: cowAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "h55EUEsTG9224",
        iframeWidth: "480",
        iframeHeight: "317",
        aHref: "https://giphy.com/gifs/reactiongifs-h55EUEsTG9224",
      },

      {
        iframeSrc: "lJzjS0e4DrFIs",
        iframeWidth: "480",
        iframeHeight: "360",
        aHref: "https://giphy.com/gifs/cow-lJzjS0e4DrFIs",
      },
    ],
  },

  {
    id: "word-20",
    word: "worm",
    chestMin: "RARE",
    audio: wormAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "9ZGwMvzxz2SNa",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/puffinrock-wiggle-worm-9ZGwMvzxz2SNa",
      },

      {
        iframeSrc: "xTiTntdVHPOnIhK608",
        iframeWidth: "480",
        iframeHeight: "360",
        aHref:
          "https://giphy.com/gifs/inch-rubber-hose-worm-xTiTntdVHPOnIhK608",
      },
    ],
  },

  {
    id: "word-21",
    word: "orca",
    chestMin: "EPIC",
    audio: wormAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "SUi7WCnbmT5Zu",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/qz-SUi7WCnbmT5Zu",
      },

      {
        iframeSrc: "ZzDd1Xiejorok",
        iframeWidth: "480",
        iframeHeight: "323",
        aHref: "https://giphy.com/gifs/orca-ZzDd1Xiejorok",
      },
    ],
  },

  {
    id: "word-22",
    word: "moon",
    chestMin: "EPIC",
    audio: wormAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "siCRldvfdu3Ic",
        iframeWidth: "480",
        iframeHeight: "273",
        aHref: "https://giphy.com/gifs/moon-dark-siCRldvfdu3Ic",
      },

      {
        iframeSrc: "i1iA5wwEuRQNq",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/moon-black-and-white-i1iA5wwEuRQNq",
      },
    ],
  },

  {
    id: "word-23",
    word: "water",
    chestMin: "EPIC",
    audio: wormAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "vnNHyroyEwb4s",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/water-rainfall-vnNHyroyEwb4s",
      },

      {
        iframeSrc: "VpmPTHI4ZwTm0",
        iframeWidth: "480",
        iframeHeight: "263",
        aHref: "https://giphy.com/gifs/water-beach-summer-VpmPTHI4ZwTm0",
      },
    ],
  },

  {
    id: "word-24",
    word: "bug",
    chestMin: "EPIC",
    audio: wormAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "44s6SAe8q4X8A",
        iframeWidth: "480",
        iframeHeight: "269",
        aHref: "https://giphy.com/gifs/interesting-bug-puppet-44s6SAe8q4X8A",
      },
    ],
  },

  {
    id: "word-25",
    word: "nest",
    audio: nestAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "RFekv4SvVuwy4",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref: "https://giphy.com/gifs/bird-robin-RFekv4SvVuwy4",
      },
    ],
  },

  /* Was supposed to be she'll */

  // {
  //   id: "word-26",
  //   word: "shell",
  //   audio: shellAudioUrl,
  //   giphyEmbeds: [
  //     {
  //       iframeSrc: "1jkSrLRJnCE6yQcHU9",
  //       iframeWidth: "480",
  //       iframeHeight: "480",
  //       aHref: "https://giphy.com/gifs/shell-loveboard-1jkSrLRJnCE6yQcHU9",
  //     },
  //   ],
  // },

  {
    id: "word-27",
    word: "one",
    audio: oneAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "l0ExncehJzexFpRHq",
        iframeWidth: "480",
        iframeHeight: "320",
        aHref:
          "https://giphy.com/gifs/studiosoriginals-numbers-gilphabet-l0ExncehJzexFpRHq",
      },
    ],
  },

  {
    id: "word-28",
    word: "shop",
    audio: shopAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "26xBDL9X2vhjHYVRm",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref:
          "https://giphy.com/gifs/target-bullseye-targetstyle-26xBDL9X2vhjHYVRm",
      },
    ],
  },

  {
    id: "word-29",
    word: "said",
    audio: saidAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "3og0IBJugq2uf9lGwg",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/yandel-roc-nation-3og0IBJugq2uf9lGwg",
      },
    ],
  },

  {
    id: "word-30",
    word: "ask",
    audio: askAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "xT0xeuDzFpbs3GPoD6",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref:
          "https://giphy.com/gifs/true-and-the-rainbow-kingdom-cat-netflix-xT0xeuDzFpbs3GPoD6",
      },
    ],
  },

  {
    id: "word-31",
    word: "best",
    audio: bestAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "26u4cqiYI30juCOGY",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/awkwafina-26u4cqiYI30juCOGY",
      },
    ],
  },

  {
    id: "word-32",
    word: "desk",
    audio: deskAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "11dR2hEgtN5KoM",
        iframeWidth: "460",
        iframeHeight: "480",
        aHref: "https://giphy.com/gifs/hoppip-cat-drawing-11dR2hEgtN5KoM",
      },
    ],
  },

  {
    id: "word-33",
    word: "dust",
    audio: dustAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "3osBL580FVbU3inkd2",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref:
          "https://giphy.com/gifs/swifferofficial-cleaning-clean-3osBL580FVbU3inkd2",
      },
    ],
  },

  {
    id: "word-34",
    word: "fast",
    audio: fastAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "mLOb56l2DQUCs",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref: "https://giphy.com/gifs/nothing-speedy-gonzales-mLOb56l2DQUCs",
      },
    ],
  },

  {
    id: "word-35",
    word: "just",
    audio: justAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "aiOKZWnTwDdxC",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref: "https://giphy.com/gifs/baby-just-chicks-aiOKZWnTwDdxC",
      },
    ],
  },

  {
    id: "word-36",
    word: "last",
    audio: lastAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "Ll9SHLJiHYWB1khhKy",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref:
          "https://giphy.com/gifs/wilder-harrier-still-training-fun-at-the-park-work-overflow-Ll9SHLJiHYWB1khhKy",
      },
    ],
  },

  {
    id: "word-37",
    word: "list",
    audio: listAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "dWOKEQ5ewh94RJLemj",
        iframeWidth: "384",
        iframeHeight: "480",
        aHref:
          "https://giphy.com/gifs/UQSportMarketing-uq-uqsport-uniofqld-dWOKEQ5ewh94RJLemj",
      },
    ],
  },

  {
    id: "word-38",
    word: "mask",
    audio: maskAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "h45pDBwEdnmtThwroC",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref:
          "https://giphy.com/gifs/potato-coronavirus-cornona-h45pDBwEdnmtThwroC",
      },
    ],
  },

  {
    id: "word-39",
    word: "must",
    audio: mustAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "ffGvcn3YRo7eN78zog",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref:
          "https://giphy.com/gifs/paramountnetwork-caroline-harris-ffGvcn3YRo7eN78zog",
      },
    ],
  },

  {
    id: "word-40",
    word: "once",
    audio: onceAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "AQ8aDPUyxKtC31Z1fX",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref: "https://giphy.com/gifs/rajasthanroyals-AQ8aDPUyxKtC31Z1fX",
      },
    ],
  },

  {
    id: "word-41",
    word: "past",
    audio: pastAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "TGLOC3kvNabio",
        iframeWidth: "480",
        iframeHeight: "360",
        aHref:
          "https://giphy.com/gifs/okkultmotionpictures-black-and-white-vintage-usa-TGLOC3kvNabio",
      },
    ],
  },

  {
    id: "word-42",
    word: "rest",
    audio: restAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "DRqOVPkytlBx6",
        iframeWidth: "480",
        iframeHeight: "392",
        aHref: "https://giphy.com/gifs/yawn-cute-DRqOVPkytlBx6",
      },
    ],
  },

  {
    id: "word-43",
    word: "test",
    audio: testAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "8FNlmNPDTo2wE",
        iframeWidth: "480",
        iframeHeight: "270",
        aHref: "https://giphy.com/gifs/me-5sf-fickledavid-8FNlmNPDTo2wE",
      },
    ],
  },

  {
    id: "word-44",
    word: "shot",
    audio: shotAudioUrl,
    giphyEmbeds: [
      {
        iframeSrc: "lPRxwkVpGja5utNeXQ",
        iframeWidth: "480",
        iframeHeight: "480",
        aHref:
          "https://giphy.com/gifs/peopleofpiramal-heart-piramal-piramalgroup-lPRxwkVpGja5utNeXQ",
      },
    ],
  },
];
